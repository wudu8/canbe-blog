/*
 * @Author: zkr
 * @Date: 2022-08-27 14:35:50
 * @LastEditors: zkr
 * @LastEditTime: 2022-09-29 16:11:26
 * @FilePath: \canbe-blog\src\locale\en-US\routes.ts
 */
export default {
  'portal.edit_article.title': '写文章',
  'portal.home.title': '博客社区',
  'portal.home_page.title': '首页',
  'portal.Webmaster_tools.title': '站长工具',
  'portal.open_source.title': '生态产品',
  'portal.about_us.title': '关于我们',
  'portal.creator.title': '创作者中心',
  'portal.creator_home.title': '首页',
  'portal.creator_content.title': '内容管理',
  'portal.creator_article.title': '文章管理',
  'portal.creator_column.title': '专栏管理',
  'portal.drafts.title': '草稿箱',
  'route.login.title': '登录',
  'route.article_detail.title': '文章详情',
  'route.msg.comment': '评论消息',
  'route.msg.like_and_store': '赞和收藏',
  'route.msg.system': '系统通知',
  'portal.personal_home.title': '个人中心',
  'portal.personal_home.info_setting': '个人信息',
  'portal.personal_home.account_setting': '账号设置'
};
